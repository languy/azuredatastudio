...
import { AzureResourcePrefixes, ResourceCategory, analyticsKind, mongoDbKind as cosmosDbMongoDbKind, cosmosDbNoSqlKind } from '../../constants';
import {
	COSMOSDB_MONGO_PROVIDER_ID, COSMOSDB_NOSQL_PROVIDER_ID, DATABASE_PROVIDER_ID, DATABASE_SERVER_PROVIDER_ID, KUSTO_PROVIDER_ID, AZURE_MONITOR_PROVIDER_ID,
...
export class AzureResourceUniversalService implements azureResource.IAzureResourceService {
	...
	public getProviderFromResourceType(type: string, kind?: string):
		...
		if ((type === azureResource.AzureResourceType.cosmosDbAccount && kind === cosmosDbMongoDbKind) || type === azureResource.AzureResourceType.cosmosDbMongoCluster) {
